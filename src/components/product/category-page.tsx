      ...
      <section className="border-b border-[#EAE5DF] bg-white">
        <div className="mx-auto grid max-w-7xl items-center gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8">
          ...
          <div className="relative hidden h-56 overflow-hidden rounded-bl-[80px] lg:block">
            <img src={heroImage} alt={`${title} collection`} className="h-full w-full object-cover" />
      ...
      <section className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          ...
          <div className="flex flex-wrap gap-3">
            {filters.map((filter) => (
              <button key={filter} className="inline-flex h-11 items-center gap-8 rounded-[4px] border border-[#EAE5DF] bg-white px-4 text-sm text-[#2D2D2D]">
                {filter}
                <ChevronDown size={15} />
              </button>
            ))}
        ...
        <div className="relative mt-7 overflow-hidden rounded-[8px] bg-[#F5F1EB]">
          <img src={editorialImages.hero} alt="Modern jewellery collection" className="h-56 w-full object-cover object-center" />
